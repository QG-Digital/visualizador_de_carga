...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const { jsPDF } = window.jspdf; // Carrega a biblioteca PDF
    ...
    let lastCalculationData = null; // Armazena os dados do último cálculo para o PDF
    ...
    function addPackageWidget(data = { name: `Tipo ${packagesListDiv.children.length + 1}`, len: 100, wid: 50, hgt: 50, qty: 10, wgt: 30, frg: 1 }) {
        ...
        // **NOVO: Lógica para renomear com duplo clique**
    ...
    function makeTitleEditable(titleElement) {
        ...
        input.type = 'text';
        input.value = oldName;
        input.className = 'package-title-input';
        ...
        const save = () => {
            ...
            newTitleElement.className = 'package-title';
    ...
    function updatePackageWidgets() {
        packagesListDiv.querySelectorAll('.package-config').forEach((widget, index) => {
    ...
    function getPackagesData() {
        return Array.from(packagesListDiv.querySelectorAll('.package-config')).map((widget, index) => {
            const data = {
                name: widget.querySelector('.package-title').textContent, // **NOVO: Pega o nome customizado**
    ...
    function calculatePositions(trucks, packages) {
        ...
        packages.forEach(pkg => {
            for (let i = 0; i < pkg.qty; i++) allItems.push({ ...pkg, vol: pkg.l * pkg.w * pkg.h });
        });
        ...
        for (const truck of trucks) {
            ...
            unplacedItems = [];
            const occupancy = new Array(Math.ceil(truck.l)).fill(0).map(() => new Array(Math.ceil(truck.w)).fill(0).map(() => new Array(Math.ceil(truck.h)).fill(null)));
            ...
            for (const item of itemsToTry) {
                ...
                for (let z = 0; z <= truck.h - item.h && !placed; z++) for (let y = 0; y <= truck.w - item.w && !placed; y++) for (let x = 0; x <= truck.l - item.l && !placed; x++) {
                    let canPlace = true;
                    for (let i = x; i < x + item.l && canPlace; i++) for (let j = y; j < y + item.w && canPlace; j++) {
                        for (let k = z; k < z + item.h; k++) if (occupancy[i][j][k] !== null) { canPlace = false; break; }
                        if (!canPlace) break;
                        if (z > 0 && isFragilityMode) { const itemBelow = occupancy[i][j][z - 1]; if (itemBelow && itemBelow.frg > item.frg) { canPlace = false; break; } }
                    }
                    if (canPlace) {
        ...
        return { placedItems, unplacedItems, status, truckStatus };
    }
    ...
    async function updateVisualization() {
        ...
        try {
            calculateBtn.disabled = true; calculateBtn.innerHTML = '⏳ Calculando...';
            showStatus('Processando...', 'info');
            await new Promise(resolve => setTimeout(resolve, 50));

            const trucks = getTruckData(); const packages = getPackagesData();
            if (packages.length === 0) throw new Error("Adicione pelo menos um tipo de pacote.");
            
            lastCalculationData = calculatePositions(trucks, packages); // Salva os dados para o PDF
            const { placedItems, unplacedItems, status, truckStatus } = lastCalculationData;

            clearScene(); drawTruck(trucks);
            ...
            calculateBtn.disabled = false; calculateBtn.innerHTML = '🧮 Calcular e Visualizar';
        }
    }
    ...
    function generatePDF() {
        ...
        const packages = getPackagesData(); // Pega os dados atuais, incluindo nomes
        ...
        doc.setFontSize(18);
        doc.text('Relatório de Carregamento 3D', 14, 22);
        ...
        doc.setFontSize(14);
        doc.text('Lista de Itens para Carregamento', 14, finalY);
        finalY += 7;

        const tableBody = packages.map(p => [
            p.name,
            `${p.l}x${p.w}x${p.h} cm`,
            p.qty,
            isWeightMode ? `${p.wgt} kg` : '-',
            isFragilityMode ? p.frg : '-'
        ]);

        const tableHeaders = [["Nome do Item", "Dimensões (CxLxA)", "Qtd", "Peso Unit.", "Frag."]];
        ...
        doc.setFontSize(14);
        doc.text('Resumo do Carregamento', 14, finalY);
        finalY += 7;

        let summaryText = [];
        status.forEach((data, name) => {
            summaryText.push(`${name}: ${data.placed} de ${data.total} alocados.`);
        });
        ...
        finalY += summaryText.length * 5;

        if (unplacedItems.length > 0) {
            finalY += 5;
            doc.setFontSize(12);
            doc.setTextColor(220, 53, 69); // Vermelho
            doc.text(`Itens Não Alocados (${unplacedItems.length})`, 14, finalY);
            doc.setTextColor(0, 0, 0);
            finalY += 7;
            
            let unplacedSummary = {};
            unplacedItems.forEach(item => {
                unplacedSummary[item.name] = (unplacedSummary[item.name] || 0) + 1;
            });
            
            let unplacedText = [];
            for (const name in unplacedSummary) {
                unplacedText.push(`- ${name}: ${unplacedSummary[name]} unidade(s)`);
            }
            doc.setFontSize(10);
            doc.text(unplacedText, 14, finalY);
        }
    ...
    function setupTooltips() {
        document.querySelectorAll('.tooltip-icon').forEach(icon => {
            icon.addEventListener('mouseenter', (e) => {
                ...
                activeTooltip.className = 'tooltip-instance';
